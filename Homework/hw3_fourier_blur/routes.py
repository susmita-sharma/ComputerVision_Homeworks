...
def _run_one_kernel(image, kernel_type, ksize, sigma, stamp):
    """Runs the spatial-vs-frequency comparison for a single kernel type and
    saves ONE PNG for it - original, spatial blur, frequency blur, their
    difference, and both spectra, all in one panel - named after the kernel
    type so a run produces exactly two output images: one for gaussian, one
    for box.
    """
    ...
    comparison_url = _save_png(comparison_panel, f"{kernel_type}_result", stamp)
